...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-gray-900">{{ $storyCount ?? 0 }}</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-gray-900">{{ $bankCount ?? 0 }}</p>
    ...
    <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            @php
            $links = [
                ['route' => 'admin.wedding.edit', 'label' => 'Konfigurasi', 'hover' => 'group-hover:text-amber-600'],
                ['route' => 'admin.events.index', 'label' => 'Event', 'hover' => 'group-hover:text-blue-600'],
                ['route' => 'admin.stories.index', 'label' => 'Love Story', 'hover' => 'group-hover:text-rose-600'],
                ['route' => 'admin.galleries.index', 'label' => 'Gallery', 'hover' => 'group-hover:text-indigo-600'],
                ['route' => 'admin.gifts.index', 'label' => 'Wedding Gift', 'hover' => 'group-hover:text-emerald-600'],
                ['route' => 'admin.wishes.index', 'label' => 'Ucapan', 'hover' => 'group-hover:text-violet-600'],
                ['route' => 'admin.assets.index', 'label' => 'Asset', 'hover' => 'group-hover:text-cyan-600'],
            ];
            @endphp
            ...
            <a href="{{ route($link['route']) }}" class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 hover:shadow-md hover:border-gray-300 transition-all duration-200 group">
                <p class="text-sm font-medium text-gray-800 {{ $link['hover'] }} transition-colors">{{ $link['label'] }}</p>
                <p class="text-xs text-gray-400 mt-1">Kelola &rarr;</p>
            </a>
            @endforeach
        </div>
    </div>
