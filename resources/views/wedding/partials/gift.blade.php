<section id="gift" class="py-20 md:py-32 section-bg overlay-warm" style="background-image: url('{{ asset('images/hero/gift-bg.jpg') }}')">
    <div class="relative z-10 max-w-3xl mx-auto px-6">
        ...
        <h2 class="font-serif text-3xl md:text-5xl text-white font-light tracking-wide text-center text-shadow mb-4" data-reveal="fade" data-reveal-delay="100">Wedding Gift</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="glass-rose rounded-2xl p-6 md:p-8 shadow-2xl accent-bar-top" data-reveal="{{ $i == 0 ? 'left' : 'right' }}" data-reveal-delay="{{ $i * 200 }}">
                <p class="font-serif text-xl text-rose-light font-light text-shadow">{{ $bank->bank_name }}</p>
                <p class="font-mono text-xl md:text-2xl text-champagne/80 my-3 bank-number font-light tracking-wider" data-full="{{ $bank->account_number }}">
                    {{ App\Helpers\BankHelper::mask($bank->account_number) }}
                </p>
                <p class="text-champagne/40 text-xs font-sans font-light">a.n. {{ $bank->account_holder }}</p>
